import { bindObservable, Cookie, RequestMapper, ResponseMapper, Transformer, Connection } from 'easyhard-bridge'
import { map } from 'rxjs/operators'
import { BodyListeners, CookieSetters, HttpRequest, ReqListeners, SetCookie } from './http'

export const requestTransformer = new Transformer<RequestMapper, 1, 2, { ws: Connection<unknown, unknown>, reqListeners: ReqListeners, bodyListeners: BodyListeners }>({
  __ob: (args, { ws }) => {
...
export const responseTransformer = new Transformer<ResponseMapper, 0, 1, { ws: Connection<unknown, unknown>, cookieSetters: CookieSetters }>({
